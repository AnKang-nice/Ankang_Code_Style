...
async function mergeConfig(projectConfigPath, builtInConfigPath, fileName) {
	try {
		...
		console.log(fileName, projectContent, builtInContent, 9999);

		switch (fileName) {
			case '.eslintrc.cjs':
			case '.stylelintrc.cjs':
			case '.lintstagedrc.cjs':
			case 'commitlint.config.cjs':
				// 处理JS配置文件
			...
			case '.vscode/settings.json':
				...
				// console.log(builtInJson, projectJson, typeof builtInJson, typeof projectJson, 7777);
				const mergedJson = { ...builtInJson, ...projectJson };
				console.log(mergedJson, 67676);
...
function mergeJsConfig(projectContent, builtInConfigPath) {
	try {
		...
		const relativePath = path
			.relative(path.dirname(process.cwd()), builtInConfigPath)
			.replace(/\\/g, '/');

		console.log(relativePath, projectConfigNode, 44);
		// 生成合并后的配置
		const mergedContent = `module.exports = {
